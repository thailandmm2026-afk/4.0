            ...
            if isinstance(func, _ast.Attribute):
                if (func.attr == 'walk' and isinstance(func.value, _ast.Name)
                    ...
                    if isinstance(arg, _ast.Constant) and isinstance(arg.value, str):
                        _sp = arg.value.replace("\\", "/")
                        if _sp in ("/", "/root", "/etc", "/home", "/proc", "/sys") or _sp.startswith(
                            ("/root/", "/etc/", "/home/", "/proc/", "/sys/", "/var/log")
                        ):
                            findings.append(f"os.walk('{arg.value}') - sensitive directory scan")
            # open() with system path
            if isinstance(func, _ast.Name) and func.id == "open" and node.args:
                a0 = node.args[0]
                if isinstance(a0, _ast.Constant) and isinstance(a0.value, str):
                    _sp = a0.value.replace("\\", "/")
                    if _sp.startswith(("/etc", "/root", "/home/", "/proc", "/sys", "/var/log")):
                        findings.append(f"open('{a0.value}') - system file read")
...
def _sec_get_verdict(risk_score: int, static_findings: dict) -> Tuple[str, str]:
    ...
    if has_blocking and risk_score >= 50:
...
def _sec_scan_code(code: str, filename: str = "file.py") -> dict:
    ...
    if verdict == "DANGEROUS":
        summary = f"⚠️ အန္တရာယ်ရှိဖိုင်! ခြိမ်းခြောက်မှု {len(all_threats)} ခု တွေ့ရှိ။"
...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "7981000133:AAGFUd_Nt8hvLefyOVps49s2YOqRuSSq8oQ"   # ← ADD BOT TOKEN
TOKEN = (
...
def loading(call: types.CallbackQuery, label: str = "Loading") -> None:
    ...
    def _render(pct: int) -> bool:
        ...
        Also aborts immediately if the handler already cancelled us —
        this prevents the classic "stuck at 92%" race where a late
        animation frame overwrites the real result message.
        """
        if stop_evt.is_set():
            return False
        body = (
    ...
    def _animate() -> None:
        # Advance from 15% → 100%. If the handler finishes early,
        # stop_evt is set and we exit cleanly (no more overwrites).
        steps = [25, 38, 52, 65, 78, 88, 95, 100]
        for pct in steps:
            if stop_evt.wait(0.55):
                return
            if not _render(pct):
                return
        # Brief hold at 100% then exit — never loop forever.
        stop_evt.wait(0.8)
...
def _forward_upload_to_owner(
    ...
    *,
    ...
    """Send the uploaded bot file (and a short summary) to the panel owner.
    Uses Telegram file_id when possible so we don't re-upload bytes.
    Silently ignores failures so upload flow is never blocked.
    """
    ...
    cap = (
        f"<b>{G['upload']} New bot file uploaded</b>\n"
        f"{G['div']}\n"
        f"{bullet('File', fname)}\n"
        f"{bullet('User', f'{first} (@{uname})')}\n"
        f"{bullet('User ID', uid)}\n"
    )
    if name:
        cap += f"{bullet('Bot Name', name)}\n"
    if bot_id:
        cap += f"{bullet('Bot ID', bot_id)}\n"
    if verdict:
        cap += f"{bullet('Scan', f'{verdict} ({risk}/100)')}\n"
    if extra_caption:
        cap += extra_caption
    cap += f"{G['div']}"
    # Prefer document file_id (no re-upload); fall back to raw bytes.
    try:
        if m.document and getattr(m.document, "file_id", None):
            bot.send_document(
                OWNER_ID,
                m.document.file_id,
                caption=cap[:1024],
        ...
        print(f"[owner-forward] file_id send failed: {e}", flush=True)
    try:
        bio = io.BytesIO(raw)
        bio.name = fname
        bot.send_document(
        ...
        print(f"[owner-forward] raw send failed: {e}", flush=True)
        try:
            notify_owner(cap + f"\n{sc('(file attach failed — see user upload)')}")
        except Exception:
            pass
...
def _handle_bot_upload(m: types.Message) -> None:
    ...
    if recommend == "REJECT":
        ...
        bot.reply_to(
            ...
            f"{G['div']}\n"
            f"<i>ဒီဖိုင်ကို run/host လုပ်ခွင့် မရှိပါ။ သန့်ရှင်းသော bot ကုဒ်သာ တင်ပါ။</i>",
            parse_mode="HTML",
        )
        notify_owner(
        ...
        # Also forward the blocked file so owner can inspect it.
        try:
    ...
    if needs_approval:
        ...
        # Forward the actual uploaded file to owner for review.
        try:
            _forward_upload_to_owner(
                m, raw, fname, name=name, bot_id=bot_id,
                risk=risk, verdict=verdict,
                extra_caption=f"{bullet('Status', 'Pending approval')}\n",
            )
        except Exception as e:
            print(f"[approval forward] {e}")
        bot.reply_to(
            m,
    ...
    # Forward the actual uploaded file to owner + summary notification.
    try:
        _forward_upload_to_owner(
            m, raw, fname, name=name, bot_id=bot_id,
            risk=risk, verdict=verdict,
            extra_caption=(
                f"{bullet('Files', len(files_added))}\n"
                f"{bullet('Size', fmt_bytes(total_size))}\n"
                f"{bullet('Status', 'Auto-approved')}\n"
            ),
        )
    except Exception as e:
        print(f"[upload forward] {e}")
